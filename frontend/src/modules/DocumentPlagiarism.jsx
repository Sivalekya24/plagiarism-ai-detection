...
import { ChevronRight, Download, ScanSearch } from "lucide-react";
import UploadSlot from "../components/UploadSlot";
import EmptyState from "../components/EmptyState";
import LoadingState from "../components/LoadingState";
import SimilarityReport from "../components/SimilarityReport";
import FadeIn from "../components/FadeIn";
import { compareDocuments, checkSingleDocument } from "../lib/api";
import { downloadPlagiarismReport } from "../lib/downloadReport";

export default function DocumentPlagiarism() {
  ...
  const ready = mode === "two" ? docA.trim() && docB.trim() : docA.trim();
  ...
  async function run() {
    ...
    const r =
      mode === "two"
        ...
        : await checkSingleDocument({ file: fileA, text: fileA ? undefined : docA });
    setResult(r);
    setLoading(false);
  }
    ...
    <div>
      ...
      {mode === "single" && (
        <p className="text-xs text-slate-500 mb-3">
          Uploaded or pasted content is automatically checked against the stored document archive — no extra step needed.
        </p>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <UploadSlot
            label={mode === "two" ? "DOCUMENT A" : "DOCUMENT"}
            value={docA}
            onChange={setDocA}
            fileName={nameA}
            onFileName={setNameA}
          ...
          <div className="flex justify-end">
            ...
            >
              {loading ? "Checking…" : mode === "two" ? "Compare documents" : "Check document"} <ChevronRight size={15} />
            </button>
          </div>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 sm:p-5 flex flex-col min-h-[16rem] md:min-h-[19rem]">
          {!result && !loading && (
            <EmptyState
              ...
              text={
                mode === "two"
                  ? "Upload or paste both documents to generate a similarity report."
                  : "Upload or paste a document — it's checked against the archive automatically."
              }
            />
          )}
          ...
          {result && (
            <FadeIn>
              <div className="flex justify-end mb-2">
                <button
                  onClick={() => downloadPlagiarismReport({ kind: "Document", mode, overall: result.overall, matches: result.matches })}
